...
class HearstPatterns(object):
    ...
    def chunk(self, rawtext):
        ...
        for sentence in doc.sents:
            #lemmitise the string
            # print(sentence)
            sentence_text = sentence.lemma_
            # extract noun ffrom the text line
            for chunk in sentence.noun_chunks:
                # print("chunk =",chunk,"=")
                if chunk.lemma_.lower() == "which":
                    continue
                ...
                for token in chunk:
                    # print("token = ",token)
                    # if word is present in adj stopwords then skip
    ...
    def find_hyponyms(self, rawtext):
        ...
        for sentence in np_tagged_sentences:
            ...
            for (hearst_pattern, parser) in self.__hearst_patterns:
                # print(hearst_pattern, sentence)
                matches = re.search(hearst_pattern, sentence)
                # print(matches)
                if matches:
                    match_str = matches.group(0)
                    # print("hello = ",match_str)
